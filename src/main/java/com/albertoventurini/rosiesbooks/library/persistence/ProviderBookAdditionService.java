...
public class ProviderBookAdditionService {
  ...
  public Optional<LocalEdition> findByIsbn13(String normalizedIsbn13) {
    return editions
        .findByIsbn(Isbn13.parse(normalizedIsbn13))
        .map(
            edition -> {
              var row =
                  dsl.select(EDITION.TRUSTED_COVER_SOURCE, COVER_ASSET.SHA256)
                      .from(EDITION)
                      .leftJoin(COVER_ASSET)
                      .on(COVER_ASSET.ID.eq(EDITION.COVER_ASSET_ID))
                      .where(EDITION.ID.eq(edition.id().value()))
                      .fetchSingle();
              return LocalEdition.from(
                  edition,
                  Optional.ofNullable(row.get(EDITION.TRUSTED_COVER_SOURCE)),
  ...
  public AddedBook addProvider(CurrentUser owner, ProviderCandidate candidate, ReadingState state) {
    ...
    lock(
        "provider:"
            + candidate.lookupIsbn13()
            + ':'
            + candidate.providerName()
            + ':'
            + candidate.providerEditionId());
    Optional<Edition> byIsbn = editions.findByIsbn(Isbn13.parse(candidate.lookupIsbn13()));
    Optional<Edition> byProvider =
        findByProvider(candidate.providerName(), candidate.providerEditionId());
    if (byIsbn.isPresent()
        && byProvider.isPresent()
        && !byIsbn.get().id().equals(byProvider.get().id()))
  ...
  private Edition create(ProviderCandidate candidate) {
    ...
    CanonicalIsbns isbns =
        new CanonicalIsbns(metadata.isbn10(), Optional.of(Isbn13.parse(candidate.lookupIsbn13())));
            ...
            metadata
                ...
                .orElse(
                    com.albertoventurini.rosiesbooks.library.internal.PartialPublicationDate
                        .unknown()),
            metadata.pageCount().orElse(null),
  ...
  private Optional<Edition> findByProvider(String name, String id) {
    return dsl.select(EDITION.ID)
        ...
        .fetchOptional(row -> new EditionId(row.get(EDITION.ID)))
        .flatMap(editions::find);
  }

  private AddedBook link(CurrentUser owner, UUID editionId, ReadingState state) {
    AddedBook existing =
        dsl.select(USER_EDITION.ID, USER_EDITION.STATE)
            .from(USER_EDITION)
            .where(
                USER_EDITION
                    .USER_ID
    ...
    var inserted =
        dsl.insertInto(USER_EDITION)
            ...
            .set(USER_EDITION.STATE, fields.state())
            .set(USER_EDITION.STARTED_ON, fields.startedOn())
            .set(USER_EDITION.FINISHED_ON, fields.finishedOn())
            ...
            .set(USER_EDITION.CREATED_AT, now.atOffset(ZoneOffset.UTC))
            .set(USER_EDITION.UPDATED_AT, now.atOffset(ZoneOffset.UTC))
  ...
  public record ProviderCandidate(
    ...
    public ProviderCandidate {
      Objects.requireNonNull(lookupIsbn13);
      providerName =
          providerName == null ? "" : providerName.strip().toLowerCase(java.util.Locale.ROOT);
      providerEditionId = providerEditionId == null ? "" : providerEditionId.strip();
      Objects.requireNonNull(metadata);
      Objects.requireNonNull(cover);
      if (providerName.isEmpty() || providerEditionId.isEmpty())
        throw new IllegalArgumentException("Provider identity is required");
      Isbn13.parse(lookupIsbn13);
    }
    ...
    public ProviderCandidate(
        ...
        String providerEditionId,
        EditionMetadata metadata) {
      this(lookupIsbn13, providerName, providerEditionId, metadata, Optional.empty());
    }
  }

  public record LocalEdition(
      UUID id,
      EditionMetadata metadata,
      boolean hasCover,
      Optional<TrustedCoverReference> trustedCoverSource,
      String coverHash) {
    static LocalEdition from(Edition edition, Optional<String> source, String coverHash) {
      return new LocalEdition(
          edition.id().value(),
          edition.metadata(),
          edition.coverAssetId() != null,
          source.flatMap(
              value -> {
                try {
                  return Optional.of(new TrustedCoverReference(java.net.URI.create(value)));
  ...
  public record AddedBook(UserEditionId id, String state, UUID editionId) {}
