...
class BookEditResource {
  ...
  BookEditResource(
      ...
      EditableBookCatalog books,
      MetadataOverrideService edits,
      Clock clock,
  ...
  public TemplateInstance form(@PathParam("id") String rawId) {
    ...
    return render(
        owner,
        id,
        BookEditForm.from(book.effectiveMetadata(), book.privateNotes(), book.overrides()));
  }
  ...
  public Response submit(
    ...
    if (requested.startsWith("remove-author-")) {
      ...
      return index >= 0 && index < submitted.bibliography().authors().size()
          ? ok(owner, id, submitted.removeAuthor(index))
          : badRequest(owner, id, submitted, "Choose one of the available form actions.");
    }
    if (requested.startsWith("reset-")) {
      MetadataOverrides reset = reset(book.overrides(), requested.substring("reset-".length()));
      if (reset == null)
        return badRequest(owner, id, submitted, "Choose one of the available form actions.");
    ...
    if (normalizedNotes != null && normalizedNotes.length() > 10_000) {
      prepared =
          prepared.withErrors(
              merge(
                  prepared.errors(), "notes", "Private notes must be 10000 characters or fewer."));
    }
    if (!validation.valid() || prepared.hasError("notes"))
      return badRequest(owner, id, prepared, null);
  ...
  private EditableBook find(CurrentUser owner, UserEditionId id) {
    return books.find(owner, id).orElseThrow(BookEditResource::notFound);
  }

  private TemplateInstance render(CurrentUser owner, UserEditionId id, BookEditForm form) {
    return BookEditTemplates.edit(
        new BookEditPage(owner.displayLabel(), route(id), detailRoute(id), form));
  }

  private Response ok(CurrentUser owner, UserEditionId id, BookEditForm form) {
    return Response.ok(render(owner, id, form)).build();
  }

  private Response badRequest(
      CurrentUser owner, UserEditionId id, BookEditForm form, String error) {
  ...
  private static MetadataOverrides proposed(EditableBook book, EditionMetadata submitted) {
    ...
    return new MetadataOverrides(
        ...
        choiceOptional(
            old.publicationDate(), effective.publicationDate(), submitted.publicationDate()),
  ...
  private static <T> MetadataOverride<T> choice(MetadataOverride<T> old, T effective, T submitted) {
    if (old.isInherited() && java.util.Objects.equals(effective, submitted))
  ...
  private static <T> MetadataOverride<T> choiceOptional(
    ...
    return submitted
        .<MetadataOverride<T>>map(MetadataOverride::value)
        .orElseGet(MetadataOverride::blank);
  }
    ...
    return switch (field) {
      case "title" ->
          new MetadataOverrides(
              (MetadataOverride<String>) inherited,
              o.subtitle(),
              o.authors(),
              o.format(),
              o.isbn10(),
      ...
      case "authors" ->
          new MetadataOverrides(
              o.title(),
              o.subtitle(),
              (MetadataOverride<List<String>>) inherited,
              o.format(),
              o.isbn10(),
              o.isbn13(),
              o.publisher(),
      ...
      case "page-count" ->
          new MetadataOverrides(
              ...
              o.publisher(),
              o.publicationDate(),
              (MetadataOverride<Integer>) inherited,
              o.language(),
              o.description());
      case "language" ->
          new MetadataOverrides(
  ...
  private static String normalizeNotes(String notes) {
    String value = notes == null ? "" : notes.replace("\r\n", "\n").replace('\r', '\n');
  ...
  private static Map<String, List<String>> merge(
    ...
    java.util.LinkedHashMap<String, List<String>> result = new java.util.LinkedHashMap<>(errors);
    result.put(field, List.of(message));
    return result;
  }
  ...
  private static UserEditionId parse(String raw) {
    try {
      UUID id = UUID.fromString(raw == null ? "" : raw);
      if (!id.toString().equalsIgnoreCase(raw)) throw new IllegalArgumentException();
      return new UserEditionId(id);
    } catch (IllegalArgumentException e) {
      throw notFound();
    }
  }

  private static String route(UserEditionId id) {
    return "/books/" + id.value() + "/edit";
  }

  private static String detailRoute(UserEditionId id) {
    return "/books/" + id.value();
  }
