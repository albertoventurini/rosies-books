...
This document records the pinned starting point for task 0-1. A pin is owned either directly by a
Maven property, by the imported Quarkus platform BOM, by the checksum-verified wrapper, or by the
future Compose file named below. Quarkus-managed dependency versions are not restated on
dependencies when the BOM already supplies the selected version.
